...
contatosRouter.put("/contatos/:id", async (req, res) => {
  const { error, value } = contatoUpdateValidation.validate(req.body, {
    abortEarly: false,
  })
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Um erro ocorreu ao atualizar o contato", error: err })
  }
})
...
contatosRouter.delete("/contatos/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Um erro ocorreu ao excluir o contato", error: err })
  }
})
